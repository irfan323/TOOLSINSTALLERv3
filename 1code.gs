...
const SPREADSHEET_ID  = '1jXIjVkYCiXEYlK0ID8619h4-lQuGyyn6ZKjADqp2tUs';
const DRIVE_FOLDER_ID = '1_78f5DFEZuO83mpen3OjZQBi0mPcQK1x';
...
/** Zona waktu tampilan spreadsheet (di-cache sekali per eksekusi). */
var _tzSpreadsheet = null;
function tzSpreadsheet() {
  if (!_tzSpreadsheet) {
    try { _tzSpreadsheet = getSpreadsheet().getSpreadsheetTimeZone() || ZONA_WAKTU; }
    catch (e) { _tzSpreadsheet = ZONA_WAKTU; }
  }
  return _tzSpreadsheet;
}

/**
 * Normalisasi nilai jam ke format "HH:mm".
 * Google Sheets kerap mengubah teks "07:30" menjadi objek Date. Fungsi ini
 * mengembalikannya menjadi string "HH:mm" agar jam kerja tidak "hilang" saat
 * disimpan/dibaca dan agar jendela absensi tetap dihitung dengan benar.
 */
function normalisasiJam(v) {
  if (v === null || v === undefined || v === '') return '';
  if (Object.prototype.toString.call(v) === '[object Date]') {
    return Utilities.formatDate(v, tzSpreadsheet(), 'HH:mm');
  }
  const s = String(v).trim();
  const m = s.match(/(\d{1,2}):(\d{2})/);   // tangani juga string tanggal seperti "... 07:30:00 GMT+..."
...
/**
 * CATATAN KINERJA
 * Setiap panggilan google.script.run adalah eksekusi baru, sehingga variabel
 * global berikut otomatis segar per-permintaan (tidak ada risiko basi antar
 * permintaan). Cache ini menghapus pembacaan sheet berulang yang sebelumnya
 * terjadi ratusan kali di dalam perulangan rekap — penyebab utama aplikasi
 * terasa sangat berat. Fungsi simpan/hapus terkait mengosongkan cache-nya.
 */
var _cacheBatch = null, _cacheUnit = null, _cacheJabatan = null;

function petaBatch() {
  if (_cacheBatch) return _cacheBatch;
  const K = KOL.batch;
...
function cariBatch(kodeBatch) {
  if (!kodeBatch) return null;
  return petaBatch()[String(kodeBatch).trim().toUpperCase()] || null;
}

function petaUnit() {
  if (_cacheUnit) return _cacheUnit;
  const K = KOL.unit;
  const data = bacaSheet('unit');
  const peta = {};
  for (let i = 1; i < data.length; i++) {
    const id = String(data[i][K.id]).trim().toUpperCase();
    if (!id) continue;
    peta[id] = {
...
function cariUnit(idUnit) {
  ...
  return petaUnit()[String(idUnit).trim().toUpperCase()] || null;
}

function petaJabatan() {
  if (_cacheJabatan) return _cacheJabatan;
  const K = KOL.jabatan;
...
function cariJabatan(kodeJabatan) {
  ...
  return petaJabatan()[String(kodeJabatan).trim().toUpperCase()] || null;
}

/** Nama jabatan yang terbaca manusia; jatuh kembali ke kodenya bila tak ditemukan. */
function namaJabatan(kode) {
  if (!kode) return '-';
  const j = petaJabatan()[String(kode).trim().toUpperCase()];
  return j ? String(j.nama) : String(kode);
}
...
/** Peta jam kerja per indeks hari (0=Minggu .. 6=Sabtu), di-cache per eksekusi. */
var _cacheJamKerja = null;
function petaJamKerja() {
  if (_cacheJamKerja) return _cacheJamKerja;
  const K = KOL.jamKerja;
  const data = bacaSheet('jam_kerja');
  const peta = {};
  for (let i = 1; i < data.length; i++) {
    const h = Number(data[i][K.hari]);
    if (isNaN(h)) continue;
    peta[h] = {
      hari: h, namaHari: String(data[i][K.namaHari] || NAMA_HARI[h] || ''),
      hariKerja: String(data[i][K.hariKerja] || 'Tidak') === 'Ya',
      jamMasuk: normalisasiJam(data[i][K.jamMasuk]),
      jamPulang: normalisasiJam(data[i][K.jamPulang]),
...
function getJamKerja(indeksHari) {
  const h = Number(indeksHari);
  const jk = petaJamKerja()[h];
  if (jk) return jk;
  return { hari: h, namaHari: NAMA_HARI[h] || '', hariKerja: false,
  ...
  return jalankan(function () {
    ...
    (baris || []).forEach(function (b) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        const rng = sheet.getRange(i + 1, 1, 1, SKEMA.jam_kerja.length);
        // Paksa kolom jam menjadi TEKS agar Sheets tidak mengubah "07:30" jadi
        // objek waktu — inilah sebab jam kerja seolah "tidak bisa disimpan".
        rng.setNumberFormats([['0', '@', '@', '@', '@', '@', '@']]);
        rng.setValues([[
          h, NAMA_HARI[h], b.hariKerja ? 'Ya' : 'Tidak',
          normalisasiJam(b.jamMasuk), normalisasiJam(b.jamPulang),
          String(b.istirahat || ''), String(b.keterangan || '')
        ]]);
        break;
      }
    });
    _cacheJamKerja = null;   // segarkan cache agar perubahan langsung terbaca
...
/**
 * Perbaikan sekali jalan (dari editor Apps Script) untuk spreadsheet lama:
 * mengubah kolom jam menjadi TEKS dan menulis ulang jam masuk/pulang dalam
 * format "HH:mm". Jalankan bila jam kerja tampak kosong atau absensi menolak
 * dengan pesan "Jam kerja ... belum diatur".
 */
function perbaikiJamKerja() {
  const K = KOL.jamKerja;
  const sheet = ambilSheet('jam_kerja');
  const data = sheet.getDataRange().getValues();
  if (data.length < 2) { Logger.log('Sheet jam_kerja kosong. Jalankan setupSistem() dulu.'); return; }

  const n = data.length - 1;
  // Paksa seluruh kolom teks (2..7) menjadi format teks.
  sheet.getRange(2, 2, n, SKEMA.jam_kerja.length - 1).setNumberFormat('@');

  for (let i = 1; i < data.length; i++) {
    const masuk  = normalisasiJam(data[i][K.jamMasuk]);
    const pulang = normalisasiJam(data[i][K.jamPulang]);
    sheet.getRange(i + 1, K.jamMasuk + 1, 1, 2).setValues([[masuk, pulang]]);
  }
  _cacheJamKerja = null;
  Logger.log('perbaikiJamKerja() selesai untuk ' + n + ' baris. Jam kerja kini tersimpan sebagai teks HH:mm.');
}

/** Peta hari libur {yyyy-MM-dd: keterangan}, di-cache per eksekusi. */
var _cacheLibur = null;
function petaLibur() {
  if (_cacheLibur) return _cacheLibur;
  const data = bacaSheet('hari_libur');
  const peta = {};
  for (let i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    peta[formatWaktu(data[i][0], 'yyyy-MM-dd')] = String(data[i][1] || 'Hari libur');
  }
  _cacheLibur = peta;
  return peta;
}
...
function cekHariLibur(tanggalStr) {
  return petaLibur()[tanggalStr] || null;
}

/** Hari kerja = terdaftar sebagai hari kerja pada sheet jam_kerja DAN bukan hari libur. */
function apakahHariKerja(tanggal) {
  const d = new Date(tanggal);
  const jk = petaJamKerja()[d.getDay()];
  if (!jk || !jk.hariKerja) return false;
  return !petaLibur()[formatWaktu(d, 'yyyy-MM-dd')];
}
...
function hitungHariKe(tanggalMulai, tanggalIni) {
  ...
  const libur = petaLibur();
  const jam   = petaJamKerja();
  ...
  while (kursor <= ini) {
    ...
    const jk = jam[kursor.getDay()];
    if (jk && jk.hariKerja && !libur[str]) hitung++;
...
function simpanHariLibur(token, tanggal, keterangan, jenis) {
  return jalankan(function () {
    ...
    const data = sheet.getDataRange().getValues();
    _cacheLibur = null;   // segarkan cache libur
    for (let i = 1; i < data.length; i++) {
...
function setupSistem() {
  ...
  if (jk.getLastRow() < 2) {
    // Kolom jam disimpan sebagai TEKS agar "07:30" tidak diubah Sheets jadi objek waktu.
    jk.getRange(2, 2, 7, SKEMA.jam_kerja.length - 1).setNumberFormat('@');
